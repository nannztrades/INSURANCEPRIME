...
def _active_upload_id(agent_code: str, month_year: str) -> Optional[int]:
    ...
    Falls back to latest from statement if uploads table doc_type/is_active not present.
    """
    conn = get_conn()
    try:
...
def _render_bar_chart_net_across_months(
    c: canvas.Canvas, agent_code: str, month_year: str, y: float, months_back: int = 6
) -> float:
    conn = get_conn()
    try:
        labels: List[str] = []
